...
import dev.panwar.scholarshipapp.databinding.ActivitySignUpBinding
...
//        this will take name entered by user and also trim if user entered any space after name
        ...
        val phoneNum:String=binding?.etPhoneNumber?.text.toString().trim{it<=' '}
...
//                    Storing the User in Firebase Cloud FireStore DB to use it later...for this we call register user fun of  FireStore Class
                    val user = User(id = firebaseUser.uid, name = name, email = registeredEmail, phoneNumber = phoneNum)//Other field of User data class will be Empty or with default values
